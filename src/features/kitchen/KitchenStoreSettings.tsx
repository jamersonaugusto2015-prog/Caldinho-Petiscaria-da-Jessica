...
type SettingsDraft = Pick<PublicStoreSettings, 'storeName' | 'city' | 'storeLat' | 'storeLng' | 'deliveryPricePerKm' | 'deliveryBaseFee' | 'deliveryMinFee' | 'freeDeliveryAbove' | 'maxDeliveryKm' | 'minOrderValue' | 'routeFactor' | 'driverFeePerDelivery' | 'pixKey' | 'pixMerchantName' | 'pixMerchantCity' | 'storeWhatsApp' | 'orderSoundUrl' | 'openingHours' | 'orderEnabled' | 'forceOpen' | 'backupEnabled' | 'backupFrequencyDays' | 'backupFolderId'> & { kitchenPin: string; backupServiceAccount: string };
...
export const KitchenStoreSettings: React.FC<{ activeTab: KitchenTab; setActiveTab: (tab: KitchenTab) => void }> = ({ activeTab, setActiveTab }) => {
  ...
  const save = () => {
    ...
    void saveSettings({
      ...
      ...(draft.kitchenPin.trim() ? { kitchenPin: draft.kitchenPin.trim() } : {}),
  ...
  const lookupCep = async () => {
    ...
    try {
      ...
      setAddress([result.street, result.neighborhood, result.city].filter(Boolean).join(', '));
      await locate([result.street, result.neighborhood, result.city].filter(Boolean).join(', '));
    } catch (error) {
