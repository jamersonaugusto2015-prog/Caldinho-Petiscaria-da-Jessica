...
test('computeDeliveryFee cobra a taxa mínima quando a distância é 0 ou desconhecida', () => {
  // As coordenadas vêm do corpo da requisição: distância 0 grátis deixava o
  // cliente mandar o pino da própria loja e zerar o frete.
  assert.equal(computeDeliveryFee(baseAddress, baseSettings), baseSettings.deliveryMinFee);
});
...
test('isStoreOpen usa o fuso da loja, não o do servidor', () => {
  const hours = Array(7).fill({ open: '18:00', close: '23:00' });
  const recife = { ...baseSettings, openingHours: hours, timezone: 'America/Recife' };
  const manaus = { ...baseSettings, openingHours: hours, timezone: 'America/Manaus' };
  const t = new Date('2026-08-17T21:30:00Z'); // 18:30 em Recife, 17:30 em Manaus
  assert.equal(isStoreOpen(recife, t), true);
  assert.equal(isStoreOpen(manaus, t), false);
});

test('fuso inválido gravado no banco não derruba a checagem', () => {
  const hours = Array(7).fill({ open: '00:00', close: '23:59' });
  const quebrado = { ...baseSettings, openingHours: hours, timezone: 'Fuso/Inexistente' };
  assert.equal(typeof isStoreOpen(quebrado, new Date()), 'boolean');
});
...
test('a madrugada é coberta pelo turno de ONTEM que cruzou a meia-noite', () => {
  // Só segunda (índice 1) tem turno 22h→02h; terça está sem horário. Terça de
  // madrugada (01h) tem que estar ABERTA — é o fim do turno da segunda. Antes,
  // sem horário na terça, a loja fechava à meia-noite com a cozinha aberta.
  // `timezone: ''` usa o fuso do processo nos dois lados: sem ambiguidade.
  const settings = {
    ...baseSettings,
    timezone: '',
    openingHours: baseSettings.openingHours.map((_, i) => (i === 1 ? { open: '22:00', close: '02:00' } : null)),
  };
  assert.equal(isStoreOpen(settings, new Date('2026-08-17T23:30:00')), true, 'segunda à noite: aberta');
  assert.equal(isStoreOpen(settings, new Date('2026-08-18T01:00:00')), true, 'terça de madrugada: turno da segunda');
  assert.equal(isStoreOpen(settings, new Date('2026-08-18T03:00:00')), false, 'terça 03h: já fechou');
  assert.equal(isStoreOpen(settings, new Date('2026-08-18T19:00:00')), false, 'terça à noite: sem turno próprio');
});
