...
import type { ShopId } from '../shop/types';
import { customerRoom, driverRoom, driversRoom, kitchenRoom } from '../shop/rooms';
import { isPickup } from './fulfillment';
import { orderForDriver } from './views';
 ...
 *
 * As salas saem de `contract/shop/rooms.ts` e carregam a loja. Sem o prefixo, a
 * cozinha da loja A e a da loja B entrariam na MESMA sala `kitchen` — e cada
 * pedido novo de uma cairia na tela da outra, com nome, telefone e endereço do
 * cliente, sem erro nenhum aparecendo.
 */
...
export function orderAudience(shopId: ShopId, order: Order): OrderRecipient[] {
  const recipients: OrderRecipient[] = [];

  // A cozinha vê o pedido inteiro: é ela quem cobra, atende e devolve dinheiro.
  recipients.push({ role: 'kitchen', room: kitchenRoom(shopId), order });

  if (order.driverId) {
    recipients.push({
      role: 'driver',
      room: driverRoom(shopId, order.driverId),
  ...
  if (!isPickup(order) && DRIVER_LIST_STATUSES.includes(order.status)) {
    ...
    recipients.push({
      role: 'driver',
      room: driversRoom(shopId),
      except: order.driverId ? driverRoom(shopId, order.driverId) : undefined,
  ...
  if (order.customerId && order.customerId !== 'anon') {
    recipients.push({
      role: 'client',
      room: customerRoom(shopId, order.customerId),
