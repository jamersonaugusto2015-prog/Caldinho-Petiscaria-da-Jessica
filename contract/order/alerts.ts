...
import { formatMoney, toCents } from '../pricing/money';
...
// O formatador ÚNICO de dinheiro (money.ts). A cópia local que morava aqui
// era o segundo dialeto — e imprimia "R$ NaN" onde a regra única imprime zero.
...
function kitchenAlert(event: OrderAlertEvent, order: Order, ctx: AlertContext): OrderAlert | null {
  ...
  if (pendingCancelRequest(order) && !ctx.hadPendingCancelRequest) {
    return build('kitchen', order.id, {
      ...
      title: `Cancelamento solicitado no pedido ${order.id}`,
...
export function chatAlertFor(role: AlertRole, message: ChatMessage): OrderAlert | null {
  ...
  // O motoboy também escreve no chat — e a mensagem dele saía como "A loja".
  const who = from === 'client' ? 'O cliente' : from === 'driver' ? 'O motoboy' : 'A loja';
