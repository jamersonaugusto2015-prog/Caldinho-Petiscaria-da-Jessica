...
test('não dá para voltar atrás nem repetir o status', () => {
  assert.equal(transitionProblem(delivery('pronto'), 'em_preparo', 'kitchen')?.message, 'Este pedido já passou dessa etapa.');
  assert.equal(transitionProblem(delivery('pronto'), 'pronto', 'kitchen')?.message, 'Este pedido já passou dessa etapa.');
});
