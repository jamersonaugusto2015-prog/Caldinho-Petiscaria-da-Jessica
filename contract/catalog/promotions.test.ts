...
test('duas leve-pague que disputam as mesmas linhas resolvem pela MELHOR, não pela ordem', () => {
  // 6 unidades a R$20. "leve 2 pague 1" dá 3 grátis (R$60); "leve 5 pague 4"
  // dá 1 grátis (R$20). O cliente leva sempre a melhor (60), venha na ordem que
  // vier — antes o resultado dependia da ordem do array.
  const leve2 = promo({ id: 'lp-2', kind: 'leve_pague', buyQty: 2, payQty: 1, discountPercent: undefined });
  const leve5 = promo({ id: 'lp-5', kind: 'leve_pague', buyQty: 5, payQty: 4, discountPercent: undefined });
  const carrinho = () => ctx([line({ quantity: 6, unitPrice: 20 })]);
  assert.equal(applyPromotions([leve2, leve5], carrinho()).itemDiscount, 60);
  assert.equal(applyPromotions([leve5, leve2], carrinho()).itemDiscount, 60);
});
