...
export function detectBrand(digits: string): Brand {
  ...
  if (/^2[2-7]/.test(digits)) {
    // A série 2 da Mastercard é 2221–2720 — não 2200–2799 inteira.
    if (digits.length < 4) return 'master'; // digitação incompleta: palpite
    const prefixo = Number(digits.slice(0, 4));
    return prefixo >= 2221 && prefixo <= 2720 ? 'master' : 'unknown';
  }
  return 'unknown';
}
